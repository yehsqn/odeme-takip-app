...
function formatSettings(row) {
  ...
  return {
    ...
    lastTelegramNotification: row.last_telegram_notification,
    backup: {
...
router.put('/:userId', async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `INSERT INTO settings (id, user_id, cut_off_day, telegram_bot_token, telegram_chat_id, telegram_notifications_enabled, banks, notification_days, last_telegram_notification, backup_enabled, backup_time)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET
              ...
              last_telegram_notification = excluded.last_telegram_notification,
      ...
      args: [
        ...
        telegram.notificationsEnabled !== false ? 1 : 0,
        banks,
        s.notificationDays ?? 3,
        s.lastTelegramNotification ?? null,
